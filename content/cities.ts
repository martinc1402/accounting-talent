/*
  Q4 captures the city and derives the state automatically. The list is
  comprehensive: it covers every state and union territory (capitals, district
  seats, and tier-2/3 cities) so almost any applicant finds their city and gets
  a state without typing it. The default (empty-search) suggestions still lead
  with the recruiting hubs in FEATURED_CITIES, and the "Use <typed>" fallback in
  the CityField handles anything still missing, so no one is ever blocked.

  One row per city name (first-wins on the rare name shared across states, e.g.
  Aurangabad resolves to Maharashtra), which is enough for a state hint that
  employers don't filter on.
*/
...
// Shown as the default suggestions before the applicant types anything, so the
// empty state leads with recruiting hubs rather than alphabetical firsts.
export const FEATURED_CITIES: readonly string[] = [
  "Ahmedabad", "Indore", "Kochi", "Coimbatore", "Jaipur", "Pune", "Mumbai",
