...
/*
  Internal, unlisted, robots-disallowed. Fires Email A and creates the assessment
  row. Auth + rate limit + a 404-on-anything-wrong live in requireAdmin.
...
export async function POST(request: Request) {
  ...
  let result;
  try {
    result = await sendAssessmentInvite(applicationId, baseUrl(request));
  } catch (e) {
