...
export type DisclosureRow = {
  key: "name" | "photo" | "location" | "compensation" | "employer" | "contact";
  label: string;
  value: string;
};
...
export function disclosureRows(level: VisibilityLevel): DisclosureRow[] {
  ...
  const verified = canSeeVerifiedEmployerFields(level); // photo, exact city, compensation
  ...
  return [
    ...
    { key: "location", label: "Location", value: verified ? "Exact city" : "Region only" },
    { key: "compensation", label: "Compensation", value: verified ? "Shown" : "Hidden" },
